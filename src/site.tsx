...
function SolutionsPage() {
  ...
  return <main className="subpage solutions-page">
    ...
    <section className="solution-groups section"><div className="shell">{groups.map(g=><div className="solution-group" key={g.n}><div className="solution-group-head"><span>{g.n}</span><div><small>{g.label}</small><h2>{g.title}</h2><p>{g.body}</p></div></div><div className="solution-group-links">{g.slugs.map(slug=>{const item=solutionItems.find(x=>x.slug===slug);return item ? <Link key={slug} to={`/solutions/${slug}`}><h3>{item.title}</h3><p>{collectionDescriptions[slug]}</p><b>↗</b></Link> : null})}</div></div>)}</div></section>
...
type CaseDetail = {intro:string; background:string; backgroundMore:string; approach:string; approachMore:string; built:string[]; facts:string[]; engagement?:string; team?:string; client?:string; scope?:string; technology?:string; value:string};
const caseDetails: Record<string,CaseDetail> = {
  'workflow-platform':{client:'資金調達済み日本スタートアップ',scope:'プロダクト基盤の継続開発',technology:'Web Platform / Workflow',value:'事業や業務の変化に合わせて、長期的に改善できるプロダクト基盤を開発・継続支援できることを示す事例です。',intro:'資金調達済みスタートアップのWorkflow Management Platform開発を支援しています。',background:'事業の成長や業務変化に合わせて進化する、ワークフロー管理のプロダクト基盤を継続的に開発しています。',backgroundMore:'ワークフロー系プロダクトは、事業が成長するほど扱うルールや機能が増え、初期要件のまま固定して運用することが難しくなります。そのため、短期の完成だけでなく、変更を取り込み続けられる開発体制とプロダクト基盤が重要になります。',approach:'一度作って終わるのではなく、事業側の変化を取り込みながら機能を追加・改善できるプロダクトとして支援しています。',approachMore:'Product Managerとクライアントエンジニアを含むチームに入り、継続的な機能追加・改善を前提として開発を進めています。単発の受託開発ではなく、プロダクトの変化に合わせて判断と実装を積み重ねる支援です。',built:['Workflow Management Platform','継続的な機能追加・改善','業務・サービス基盤のプロダクト開発'],facts:['2023年から継続支援','資金調達済みスタートアップ','Platform / SaaS'],engagement:'2023年3月から継続',team:'Product Manager 1名 / Client Engineer 1名 / Development Engineers 3名'},
  'dog-face-recognition-ai':{client:'日本スタートアップ',scope:'AI R&D / 技術検証から継続開発',technology:'AI / Computer Vision',value:'既製機能の導入ではなく、認識技術そのものに不確実性があるテーマでも、R&Dからプロダクト化を見据えて進められることを示す事例です。',intro:'犬の顔認識技術に関するAI開発をR&Dから支援しています。',background:'一般的な業務システムとは異なり、認識精度や実現方法そのものに技術的な不確実性があるテーマです。',backgroundMore:'犬の顔は犬種、毛並み、向き、表情などの差が大きく、人向けの顔認識と同じ前提をそのまま適用できるとは限りません。プロダクト化を考える前に、どの条件で認識できるのか、どこに難しさがあるのかを技術的に確かめる必要があります。',approach:'R&Dとして技術検証から取り組み、実現可能性を確認しながらAI機能の開発を進めています。',approachMore:'研究開発チームとして検証と実装を繰り返し、認識技術そのものを段階的に評価しています。特許準備に関わる技術テーマとして、単なる既製AIの導入ではなく、プロダクト化を見据えた継続開発を支援しています。',built:['AI / Computer Vision','顔認識に関する技術検証','プロダクト化に向けた継続開発'],facts:['2021年から継続支援','特許準備に関わる技術開発','AI / R&D'],engagement:'2021年10月から継続',team:'R&D Team / Development Engineers 3名'},
  'content-optimization':{client:'東証プライム上場企業',scope:'Web / Appプロダクトの長期開発支援',technology:'Web / App / Optimization',value:'運用中のプロダクトに対し、単発開発ではなく長期間にわたり改善を積み重ねる開発パートナーとして支援している事例です。',intro:'Web/Appのコンテンツ最適化ツールを長期にわたり開発支援しています。',background:'Webとアプリの運用・最適化を支えるツールとして、長期間にわたり継続的な開発を支援しています。',backgroundMore:'運用中のプロダクトでは、新機能を追加するだけでなく、既存機能や既存利用との整合性を保ちながら改善を続ける必要があります。長期運用ほど、局所的な改修ではなく、プロダクト全体を理解した継続的な開発が重要になります。',approach:'運用中のプロダクトとして、必要な改善や追加開発を継続しながら支援しています。',approachMore:'クライアント側のエンジニアと共同チームを構成し、2018年から継続してWeb/App領域の改善に参加しています。既存プロダクトの文脈を維持しながら追加開発を重ねる、長期協業型の支援です。',built:['Web/App Content Optimization Tool','運用を支える継続的な機能改善','Web / App向け機能開発'],facts:['東証プライム上場企業向け','2018年から継続支援','長期協業'],engagement:'2018年9月から継続',team:'Client Engineers 7名 / Development Engineers 5名'},
  'ai-chatbot':{client:'日本スタートアップ',scope:'AI機能のプロダクト組み込み',technology:'Generative AI / Web Product',value:'生成AIを単体デモで終わらせず、ユーザーが利用するプロダクト機能として組み込む開発を支援する事例です。',intro:'スタートアップ向けAIチャットボット開発を支援しています。',background:'生成AIをユーザー向けプロダクトの機能として組み込む開発テーマです。',backgroundMore:'AIチャットボットは、LLMを呼び出すだけではプロダクトとして成立しません。どの文脈をAIへ渡すか、既存機能とどう接続するか、回答をユーザー体験の中でどう扱うかまで含めて設計する必要があります。',approach:'AIそのものだけでなく、利用体験やプロダクトの一機能として成立する形で開発を支援しています。',approachMore:'クライアント側エンジニアと共同で、AI機能を既存のWebプロダクトへ組み込む前提で開発しています。AI単体のデモではなく、利用者が実際に触る機能として成立させることを重視しています。',built:['AI Chatbot','AI機能のプロダクト組み込み','Webプロダクト開発'],facts:['2025年から支援','スタートアップ','AI / Product'],engagement:'2025年5月から支援',team:'Client Engineers 3名 / Development Manager 1名 / Development Engineers 3名'},
  'ecommerce-platform':{client:'資金調達済み日本スタートアップ',scope:'ECプラットフォーム開発',technology:'E-Commerce / Web Platform',value:'事業運営を支えるマスターデータや業務機能を含め、ECを単なる販売画面ではなく事業基盤として開発する事例です。',intro:'資金調達済みスタートアップ向けE-Commerce Platformを開発支援しています。',background:'EC事業を支えるプラットフォームとして、サービス運営に必要な機能を継続的に開発しています。',backgroundMore:'ECは販売画面だけで完結せず、商品・マスターデータや運用側の業務機能がサービス全体を支えます。事業の拡張に合わせてデータや運用ルールも増えるため、フロントだけでなく基盤側を含めた設計が必要です。',approach:'事業要件に合わせて、プロダクトとして必要な機能を段階的に設計・実装しています。',approachMore:'Master Data Teamの一員として、マネージャーとエンジニアが継続的に開発へ参加しています。事業側の運用を支えるデータ・業務機能も含め、ECをサービス基盤として育てる支援です。',built:['E-Commerce Platform','サービス運営に必要な業務機能','継続的なプロダクト開発'],facts:['2023年から支援','資金調達済みスタートアップ','E-Commerce / Platform'],engagement:'2023年7月から支援',team:'Master Data Team / Development Manager 1名 / Development Engineers 3名'},
  'ios-search-ads-optimization':{client:'広告代理店',scope:'企画から導入まで',technology:'Node.js / React / Mobile',value:'広告運用データと推薦・最適化ロジックを組み合わせ、日々の運用判断を支援するシステムとして実装した事例です。',intro:'iOS Search Adsの運用最適化を支えるシステム開発に取り組みました。',background:'広告運用データを活用し、運用判断をより効率的に行うための最適化・推薦領域の開発テーマです。',backgroundMore:'Search Adsの運用では、キャンペーンやキーワードなど複数の情報を継続的に確認し、次のアクションを判断する必要があります。分析結果を表示するだけでなく、日々の運用判断につながる仕組みにすることが開発のポイントでした。',approach:'運用データを扱う仕組みと最適化ロジックを組み合わせ、実務で利用できる形へ落とし込みました。',approachMore:'キャンペーン管理、キーワードレコメンド、出稿管理、アナリティクスを一つの運用フローとして扱えるように設計しました。バックエンドのデータ処理とUIをつなぎ、担当者が継続的に判断・操作できるシステムとして実装しています。',built:['Search Ads運用支援','データを用いた最適化・推薦','運用システム開発'],facts:['Optimization','Recommendation','iOS Search Ads']},
  'advanced-rd-poc':{client:'海外統計局',scope:'実装実験 / R&D / プロトタイプ',technology:'C++ / OpenCV / OCR / IoT',value:'実現可能性がまだ明確でない先端技術テーマでも、いきなり本番開発せず、実験とプロトタイプで技術的な判断材料を作る事例です。',intro:'AI、OpenCV、OCR、IoTなどを用いた先端技術のR&D・実証実験を支援しました。',background:'実現可能性や精度を事前に確認する必要がある、不確実性の高い技術テーマを扱いました。',backgroundMore:'先端技術を使うテーマでは、最初から本番システムを設計しても、入力データや処理条件、利用環境によって期待した結果が得られない可能性があります。そこで、精度や処理条件を含む技術的な不確実性を早い段階で切り分けることが重要になります。',approach:'技術検証と小規模なプロトタイプを通じて、実用化に向けた判断材料を作りました。',approachMore:'C++、OpenCV、OCR、IoTなどを組み合わせ、小さな実装実験とプロトタイプで検証を進めました。完成品を急ぐのではなく、何が実現可能か、どの条件が制約になるか、次段階で何を検証すべきかを明確にすることを目的としたR&Dです。',built:['AI / OpenCV / OCR','IoT連携','PoC・プロトタイプ'],facts:['R&D','PoC','Emerging Technology']},
  'shift-scheduling-system':{client:'日本スタートアップ',scope:'シフト業務システム開発',technology:'Business System / Scheduling',value:'複数の勤務条件や運用ルールを、現場で継続利用できる業務システムへ落とし込む開発事例です。',intro:'シフト作成・管理業務を支えるシステムを開発しました。',background:'勤務条件や人員配置を扱うシフト業務を、継続的に運用できるシステムとして整理する開発テーマです。',backgroundMore:'シフト管理では、勤務条件や配置ルールなど複数の条件を日々の運用の中で扱う必要があります。表計算の置き換えだけではなく、現場で使われているルールをデータと操作フローへ落とし込むことが重要です。',approach:'実際のシフト管理フローをシステム化し、日常運用に必要な管理機能を実装しました。',approachMore:'管理者が繰り返し利用することを前提に、シフト作成・勤務条件・管理機能を一つの業務フローとして整理しました。個別機能よりも、日常業務の中で無理なく使い続けられる構成を重視しています。',built:['シフト作成・管理','勤務条件の管理','業務管理機能'],facts:['Business System','Scheduling','Operations']},
  'line-chat-automation':{client:'日本企業',scope:'チャット起点の業務自動化',technology:'LINE / Integration / Automation',value:'既存のユーザー接点を変えずに、チャットと後続業務を連携し、手作業を減らす仕組みを構築した事例です。',intro:'LINEを利用したChat System Automationを開発しました。',background:'チャット上のやり取りと後続の業務処理をつなぎ、手作業を減らす自動化テーマです。',backgroundMore:'ユーザーとの接点がLINEに集約されていても、その後の確認・転記・処理が人手のままだと、業務全体としては自動化されません。既存の使いやすい接点を維持しながら、後続業務までつなぐことが課題になります。',approach:'LINEをユーザー接点として業務フローと連携し、チャットから処理につながる仕組みを構築しました。',approachMore:'LINEを新しいUIへ置き換えるのではなく、そのままフロントの接点として活用し、後続のシステム処理と連携しました。利用者の操作を大きく変えずに、バックオフィス側の手作業を減らす方向で設計しています。',built:['LINE連携','Chat System','業務自動化'],facts:['Automation','LINE','Integration'],engagement:'2018年9月から継続',team:'Client Engineer 1名 / Development Manager 1名 / Development Engineers 5名'},
  'video-cv-platform':{client:'人材紹介会社',scope:'企画から導入まで',technology:'Node.js / React / Web',value:'動画、職務経歴、インタビュー、評価、決済など複数の業務・ユーザー機能を一つの採用プラットフォームとして構築した事例です。',intro:'動画を活用したCV・プロフィール領域のプラットフォームを開発しました。',background:'テキストだけでは伝わりにくい情報を動画で扱うためのデジタルプロダクト開発です。',backgroundMore:'採用サービスとして成立させるには、動画をアップロード・再生できるだけでは足りません。求職者の職務経歴、企業側の閲覧・評価、インタビュー、決済、運営管理など、採用プロセス全体を支える複数の機能が必要になります。',approach:'動画を中心としたユーザー体験とサービス運営に必要な機能を、プラットフォームとして設計・実装しました。',approachMore:'ビデオ自己紹介、職務経歴管理、インタビュー、決済、コメント・評価、分析・トラッキングを一つのWebサービスとして構成しました。動画機能を単体で作るのではなく、求職者と企業双方が利用する採用プラットフォームとして設計しています。',built:['動画CV機能','Webプラットフォーム','管理・運用機能'],facts:['Video','Platform','Product Development']},
  'stroller-sharing':{client:'ベビー用品レンタル・販売事業',scope:'企画から導入まで',technology:'Node.js / React Native / IoT',value:'モバイルアプリ、決済、在庫、物理ボックスの遠隔操作を連携させ、デジタルと現実世界をまたぐサービスを構築した事例です。',intro:'ベビーカーシェアリングサービスのシステム開発を支援しました。',background:'モバイル操作、決済、IoT・物理デバイスを一つのサービス体験として連携させる開発テーマです。',backgroundMore:'シェアリングサービスでは、アプリ上の状態だけでなく、貸出中か返却済みかという現実の状態、決済、在庫、設置ボックスの制御を一致させる必要があります。デジタルと物理の状態がずれると、サービス運営そのものに影響します。',approach:'アプリケーションと外部・物理デバイスの連携を含め、サービス運営に必要な仕組みを構築しました。',approachMore:'顧客管理、レンタル、チャット、決済、在庫管理、ボックス遠隔操作を連携し、貸出開始から返却までの一連の流れを支える構成にしました。React NativeのアプリとWeb側の機能、IoT連携をまたいだサービス開発です。',built:['モバイル機能','決済連携','IoT・物理デバイス連携'],facts:['IoT','Mobile','Sharing Service']},
  'medical-shift-management':{client:'医療法人',scope:'受託開発',technology:'Node.js / React / Web',value:'医師の勤務管理と患者予約など、現場固有の運用を整理し、日常業務を支える管理システムとして実装した事例です。',intro:'医療法人向けのシフト管理システムを開発しました。',background:'医療現場の勤務・シフト管理を、組織の運用に合わせて扱う業務システム開発です。',backgroundMore:'医療法人の勤務管理では、一般的なシフト表だけでなく、クリニック管理や支払管理など組織固有の業務と関連します。既製のテンプレートへ業務を合わせるのではなく、実際の運用に沿って必要な管理機能を整理することが重要です。',approach:'現場の管理業務を整理し、シフト情報を継続的に管理できる仕組みとして実装しました。',approachMore:'クリニック管理、シフト管理、支払管理をWebシステム上で扱えるように構成しました。医療法人の日常業務を起点に、管理者が継続運用できる業務システムとして実装しています。',built:['勤務・シフト管理','管理画面','業務システム'],facts:['Medical Organization','Business System','Shift Management']},
};
